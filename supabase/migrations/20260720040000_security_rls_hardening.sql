...
-- Lookup seguro de patrocinadora (sem expor e-mail/telefone)
...
-- Vincula parent_id uma única vez (ReferralGate)
...
DECLARE
  ...
  reseller_uuid uuid;
BEGIN
  -- parent_reseller_id opcional (UUID inválido é ignorado)
  ...
  -- Cadastro público: SOMENTE sacoleira.
  -- Nunca confiar em raw_user_meta_data->>'role' para criar admin.
  INSERT INTO public.user_roles (user_id, role)
  VALUES (NEW.id, 'sacoleira'::public.app_role)
  ON CONFLICT (user_id, role) DO NOTHING;

  -- Validar patrocinadora (se informada)
  ...
  -- Loja nasce pendente — admin aprova
...
-- Permite bootstrap do signup (auth.uid() NULL via handle_new_user DEFINER).
-- Bloqueia clientes autenticados não-admin de inserir/alterar roles.
...
BEGIN
  IF TG_OP = 'UPDATE' AND NOT public.is_admin() THEN
    ...
    IF NEW.tier IS DISTINCT FROM OLD.tier THEN
      RAISE EXCEPTION 'tier é protegido';
    END IF;
    -- parent_id: pode ser definido uma única vez (própria conta), com patrocinadora aprovada
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    -- Público / não-admin: pedido nasce como 'new' (sem pagamento real nesta etapa)
    IF auth.uid() IS NULL OR NOT public.is_admin() THEN
...
BEGIN
  IF TG_OP = 'INSERT' AND (auth.uid() IS NULL OR NOT public.is_admin()) THEN
    -- Preço vem do catálogo da loja, não do cliente
...
-- Remover triggers MLM duplicados (mantém um de cada)
...
-- Resellers: UPDATE próprio (colunas protegidas pelo trigger)
...
-- Orders: UPDATE separado (admin total; sacoleira só na própria loja — colunas protegidas)
...
-- Order items: UPDATE só admin (evita manipulação de preço)
...
-- Garantir que não há SELECT público em orders/order_items
DROP POLICY IF EXISTS "Public can view orders" ON public.orders;
DROP POLICY IF EXISTS "Public can view order items" ON public.order_items;
DROP POLICY IF EXISTS "Anyone can view orders" ON public.orders;

-- Public INSERT orders: loja aprovada (status forçado a 'new' pelo trigger)
...
-- user_roles: reforço explícito (sem INSERT para não-admin via policy)
...
-- Bootstrap signup: handle_new_user é SECURITY DEFINER (owner) e bypassa RLS.
-- Por isso o INSERT de sacoleira no signup continua funcionando.
...
-- Marketing banners path: marketing/... — só admin (já coberto por is_admin acima
-- SELECT público dos buckets permanece (leitura de imagens).
...
-- authenticated mantém SELECT completo (RLS limita linhas); sacoleira vê wholesale no catálogo
