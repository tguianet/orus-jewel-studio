...
REVOKE ALL ON TABLE public.order_reservation_settings FROM PUBLIC;
REVOKE ALL ON TABLE public.order_reservation_settings FROM anon;
...
REVOKE ALL ON FUNCTION public.get_order_reserve_minutes() FROM PUBLIC;
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      v_batch_details := COALESCE(NULLIF(current_setting('app.cancel_restore_details', true), ''), '[]')::jsonb;
...
COMMENT ON FUNCTION public.expire_abandoned_orders(integer) IS
  'Expira pedidos new/confirmed com expires_at vencido: cancelled + restore liquido. Job Lovable Cloud.';

REVOKE ALL ON FUNCTION public.expire_abandoned_orders(integer) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.expire_abandoned_orders(integer) FROM anon;
...
COMMENT ON FUNCTION public.create_public_order(uuid, text, text, text, text, jsonb, uuid) IS
  'Checkout publico atomico com expires_at (reserva). Token terminal/expirado exige novo token.';
