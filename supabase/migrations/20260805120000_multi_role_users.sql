...
-- Fonte oficial: public.user_roles (UNIQUE user_id + role).
-- Não aplicar automaticamente nesta fase (Lovable Cloud).
-- =============================================================================

-- ---------------------------------------------------------------------------
-- 1) Garantir UNIQUE (user_id, role) — sem UNIQUE(user_id) isolado
-- ---------------------------------------------------------------------------
DO $$
DECLARE
  cname text;
BEGIN
  -- Remove UNIQUE apenas em user_id (se existir), preservando dados.
  FOR cname IN
    SELECT con.conname
...
-- ---------------------------------------------------------------------------
-- 3) current_reseller_id — vínculo ao auth.uid(), independente de admin
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4) admin_list_administrators — inclui flag sacoleira / loja
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  SELECT id INTO v_reseller_id FROM public.resellers WHERE user_id = p_user_id LIMIT 1;
  v_had_reseller := v_reseller_id IS NOT NULL;

  SELECT id INTO v_store_id FROM public.seller_stores WHERE owner_user_id = p_user_id LIMIT 1;
  v_had_store := v_store_id IS NOT NULL;

  -- Slug único (exceto loja do próprio usuário)
  IF EXISTS (
  ...
  -- Role sacoleira SEM remover admin
  ...
  -- Auditoria só quando houve mudança material (role nova ou vínculo novo)
  ...
  -- Garantia: admin permanece
  IF NOT public.is_admin(p_user_id) AND EXISTS (
    SELECT 1 FROM public.admin_role_audit_log a
    WHERE a.target_user_id = p_user_id AND a.action = 'admin_granted'
  ) THEN
    NULL; -- não recria admin automaticamente se nunca foi admin
  END IF;
...
-- ---------------------------------------------------------------------------
-- 6) admin_revoke_reseller_role — remove só sacoleira; preserva admin e histórico
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Desativa acesso operacional sem apagar histórico financeiro / pedidos / loja
