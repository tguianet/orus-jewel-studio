-- =============================================================================
-- MLM por tipo de joia (gold | silver | plated) + comissão por order_item
-- Lovable Cloud — NÃO aplicar via CLI; aplicar manualmente no Cloud.
--
-- Base de cálculo auditada: order_items.total (= unit_price × quantity no checkout)
-- Compatibilidade: linhas históricas (order_item_id NULL) preservadas; unique parcial.
-- =============================================================================

-- ---------------------------------------------------------------------------
-- 1) products.jewelry_material (nullable — classificação admin obrigatória)
-- ---------------------------------------------------------------------------
DO $$ BEGIN
...
-- Bloqueia ativar produto sem material (INSERT/UPDATE)
...
-- ---------------------------------------------------------------------------
-- 2) Snapshot do material no item do pedido
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3) mlm_commission_rates — matriz material × nível
-- ---------------------------------------------------------------------------
...
-- Seed a partir de commission_settings (mesma taxa para os 3 materiais até o admin diferenciar)
...
-- ---------------------------------------------------------------------------
-- 4) commissions — colunas de snapshot por item + unique parcial
-- ---------------------------------------------------------------------------
...
-- Sync percentage_applied com rate em linhas novas se nulo
...
-- Unique legado (histórico sem item) + unique novo (por item)
...
-- ---------------------------------------------------------------------------
-- 5) RPCs de configuração da matriz
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- authenticated: taxas (sem PII); admin vê updated_by
  RETURN QUERY
...
BEGIN
  ...
  -- Mantém commission_settings em sync com taxas "gold" (compat UI/rede legada)
...
-- ---------------------------------------------------------------------------
-- 6) create_mlm_commissions_for_order — por item
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  LOOP
    ...
    -- Base auditada: order_items.total (unit_price × quantity no checkout)
...
-- reverse_mlm já itera todas as linhas do order_id — cobre itens novos sem alteração de assinatura.
...
-- ---------------------------------------------------------------------------
-- 7) Relatório admin — inclui snapshot e filtro por material
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 8) Contagem de produtos pendentes (admin)
-- ---------------------------------------------------------------------------
