...
-- Sem policy de INSERT/UPDATE/DELETE para authenticated → só RPC DEFINER

-- -----------------------------------------------------------------------------
-- 2) Perfil de recebimento (reutilizável; snapshot também em cada saque)
-- -----------------------------------------------------------------------------
...
-- Sem INSERT/UPDATE/DELETE direto (Q — RLS bloqueia update direto)
...
-- -----------------------------------------------------------------------------
-- 5) Extender wallet_transactions (tipos hold/release/paid + withdrawal_id)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 6) View: available + blocked (bloqueado = saques pending/approved)
-- -----------------------------------------------------------------------------
...
BEGIN
  -- Lock da sacoleira para serializar saques concorrentes
...
BEGIN
  ...
  IF p_amount IS NULL OR p_amount <> p_amount THEN -- NaN
...
-- -----------------------------------------------------------------------------
-- 11) reject_withdrawal (admin) — pending ou approved (sem pagamento)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 12) mark_withdrawal_paid (admin) — só approved; idempotente
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  -- Idempotência por chave de pagamento
  SELECT * INTO v_row
  FROM public.withdrawal_requests
  ...
  -- Registro de saída definitiva (amount 0 — hold já consumiu o disponível)
  IF NOT EXISTS (
    SELECT 1 FROM public.wallet_transactions
...
-- -----------------------------------------------------------------------------
-- 15) get_withdrawal_audit + get_withdrawal_details
-- -----------------------------------------------------------------------------
