...
CREATE INDEX IF NOT EXISTS idx_op_errors_correlation ON public.operational_error_logs (correlation_id);
CREATE INDEX IF NOT EXISTS idx_op_errors_occurred ON public.operational_error_logs (occurred_at DESC);
CREATE INDEX IF NOT EXISTS idx_op_errors_severity ON public.operational_error_logs (severity, occurred_at DESC);
CREATE INDEX IF NOT EXISTS idx_op_errors_category ON public.operational_error_logs (category);
CREATE INDEX IF NOT EXISTS idx_op_errors_code ON public.operational_error_logs (error_code);
CREATE INDEX IF NOT EXISTS idx_op_errors_unresolved ON public.operational_error_logs (occurred_at DESC) WHERE resolved_at IS NULL;
...
REVOKE ALL ON TABLE public.operational_error_logs FROM PUBLIC;
REVOKE ALL ON TABLE public.operational_error_logs FROM anon;
...
REVOKE ALL ON FUNCTION public._op_error_rate_ok() FROM PUBLIC;
REVOKE ALL ON FUNCTION public._op_error_rate_ok() FROM anon;
REVOKE ALL ON FUNCTION public._op_error_rate_ok() FROM authenticated;
  ...
  LOOP
    IF v_key = ANY (v_allowed) THEN
      IF jsonb_typeof(v_val) = 'object' OR jsonb_typeof(v_val) = 'array' THEN
        CONTINUE;
      ELSIF jsonb_typeof(v_val) = 'string' AND length(v_val #>> '{}') > 200 THEN
...
DECLARE
  ...
  v_recent uuid;
BEGIN
  IF p_correlation_id IS NULL OR length(trim(p_correlation_id)) < 8 OR length(trim(p_correlation_id)) > 100 THEN
    RAISE EXCEPTION 'correlation_id inválido';
  END IF;
  IF p_error_code IS NULL OR length(trim(p_error_code)) = 0 OR length(trim(p_error_code)) > 120 THEN
  ...
  END IF;
  IF p_context IS NOT NULL AND length(p_context::text) > 8000 THEN
    RAISE EXCEPTION 'contexto muito grande';
  END IF;
  ...
  FROM public.operational_error_logs
  WHERE correlation_id = trim(p_correlation_id)
    AND error_code = trim(p_error_code)
    AND occurred_at > now() - interval '30 seconds'
  LIMIT 1;

  IF v_recent IS NOT NULL THEN
    RETURN jsonb_build_object('ok', true, 'id', v_recent, 'deduplicated', true);
  END IF;
  ...
  INSERT INTO public.operational_error_logs (
    ...
    left(NULLIF(trim(COALESCE(p_operation, '')), ''), 120),
    left(NULLIF(trim(COALESCE(p_route, '')), ''), 200),
    left(NULLIF(trim(COALESCE(p_entity_type, '')), ''), 60),
    p_entity_id,
    auth.uid(),
    v_role,
    v_summary,
    public._sanitize_op_context(p_context),
    CASE WHEN length(v_headers) > 0
      THEN encode(sha256(convert_to(v_headers, 'utf8')), 'hex')
      ELSE NULL END
  )
...
REVOKE ALL ON FUNCTION public.admin_list_operational_errors(text, text, text, text, text, boolean, timestamptz, timestamptz, int, int) FROM anon;
...
REVOKE ALL ON FUNCTION public.admin_get_operational_error(uuid) FROM anon;
...
BEGIN
  ...
  SET resolved_at = COALESCE(resolved_at, now()),
      ...
      resolution_notes = left(NULLIF(trim(COALESCE(p_resolution_notes, '')), ''), 1000)
  WHERE id = p_error_id;
...
REVOKE ALL ON FUNCTION public.admin_resolve_operational_error(uuid, text) FROM anon;
