...
BEGIN
  ...
  IF v_already THEN
    RETURN jsonb_build_object('ok', true, 'already_admin', true, 'user_id', p_user_id, 'message', 'Operação já realizada');
  END IF;
  ...
  INSERT INTO public.admin_role_audit_log (
    ...
    p_user_id, 'admin_granted', NULLIF(v_prev, ''), 'admin', v_actor,
    NULLIF(trim(COALESCE(p_reason, '')), ''),
    jsonb_build_object('source', 'admin_grant_role')
  );

  RETURN jsonb_build_object('ok', true, 'already_admin', false, 'user_id', p_user_id, 'new_role', 'admin');
END;
$$;
...
BEGIN
  ...
  IF NOT v_is_admin THEN
    RETURN jsonb_build_object('ok', true, 'already_revoked', true, 'user_id', p_user_id, 'message', 'Operação já realizada');
  END IF;
  ...
  INSERT INTO public.admin_role_audit_log (
    ...
    p_user_id, 'admin_revoked', NULLIF(v_prev, ''),
    CASE
      WHEN EXISTS (
        SELECT 1 FROM public.user_roles
      ...
      ELSE NULL
    END,
    v_actor, trim(p_reason),
    jsonb_build_object('source', 'admin_revoke_role', 'self_revoke', (p_user_id = v_actor))
  );

  RETURN jsonb_build_object('ok', true, 'already_revoked', false, 'user_id', p_user_id, 'self_revoke', (p_user_id = v_actor));
END;
$$;
