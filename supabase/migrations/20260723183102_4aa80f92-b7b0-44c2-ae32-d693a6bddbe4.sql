...
BEGIN
  ...
  WHILE current_reseller IS NOT NULL AND current_level <= 3 LOOP
    ...
    IF created_commission_id IS NOT NULL THEN
      ...
      BEGIN
        ...
        SELECT current_reseller, created_commission_id, 'commission', commission_amount, 'available',
  ...
  PERFORM public.write_audit_log(
    ...
    jsonb_build_object('commissions_created',v_created_count,'wallets_created',v_wallet_count,'total',order_record.total),
    jsonb_build_object('source','create_mlm_commissions_for_order'));
END;
$$;
...
BEGIN
  IF NEW.status = 'paid'::public.order_status AND OLD.status IS DISTINCT FROM NEW.status THEN
    ...
    FROM public.commissions c
    WHERE wt.commission_id = c.id AND c.order_id = NEW.id AND wt.status='pending';
    GET DIAGNOSTICS v_wallet_count = ROW_COUNT;

    IF v_comm_count > 0 OR v_wallet_count > 0 THEN
      PERFORM public.write_audit_log(
        'release_wallet_for_paid_order','orders',NEW.id::text,
        jsonb_build_object('status',OLD.status),
        jsonb_build_object('status',NEW.status),
        jsonb_build_object('commissions_released',v_comm_count,'wallets_released',v_wallet_count,'source','release_wallet_for_paid_order'));
    END IF;
  END IF;
  RETURN NEW;
END;
...
AS $$
BEGIN
  IF TG_OP='UPDATE' AND OLD.status='paid'::public.order_status
    ...
    IF COALESCE(current_setting('app.allow_paid_reversal', true), 'off') IS DISTINCT FROM 'on' THEN
      RAISE EXCEPTION 'Pedido pago não pode mudar de status sem fluxo de estorno (app.allow_paid_reversal=on)';
    END IF;
  END IF;
  RETURN NEW;
END;
...
FOR EACH ROW EXECUTE FUNCTION public.protect_orders_paid_status();
...
REVOKE ALL ON FUNCTION public.protect_orders_paid_status() FROM PUBLIC, anon, authenticated;
...
CREATE POLICY "Admins can select commissions" ON public.commissions
FOR SELECT TO authenticated USING (public.is_admin());

CREATE POLICY "Resellers can view own commissions" ON public.commissions
FOR SELECT TO authenticated USING (public.owns_reseller(reseller_id));
...
CREATE POLICY "Admins can select wallet transactions" ON public.wallet_transactions
FOR SELECT TO authenticated USING (public.is_admin());

CREATE POLICY "Resellers can view own wallet transactions" ON public.wallet_transactions
FOR SELECT TO authenticated USING (public.owns_reseller(reseller_id));
...
COMMENT ON FUNCTION public.create_mlm_commissions_for_order(uuid) IS
  'Cria comissões MLM + wallet somente se order.status=paid. Rates 10/5/2%. ON CONFLICT evita duplicata.';