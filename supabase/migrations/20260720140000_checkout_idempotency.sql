-- =============================================================================
-- Checkout: idempotência (checkout_token) + retorno dos itens confirmados
-- Incremental, sem DROP TABLE, sem apagar dados.
-- =============================================================================

-- 1) Coluna de idempotência (nula em pedidos antigos)
...
COMMENT ON COLUMN public.orders.checkout_token IS
  'Token de idempotência do checkout público (UUID gerado no cliente).';

-- 2) Remover assinatura anterior da RPC (sem items / sem token)
...
BEGIN
  ...
  -- Cliente não pode enviar campos financeiros/status
  IF EXISTS (
    SELECT 1
...
COMMENT ON FUNCTION public.create_public_order(uuid, text, text, text, text, jsonb, uuid) IS
  'Checkout público atômico e idempotente (checkout_token). Retorna itens confirmados do banco.';

REVOKE ALL ON FUNCTION public.create_public_order(uuid, text, text, text, text, jsonb, uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.create_public_order(uuid, text, text, text, text, jsonb, uuid) FROM anon;
REVOKE ALL ON FUNCTION public.create_public_order(uuid, text, text, text, text, jsonb, uuid) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.create_public_order(uuid, text, text, text, text, jsonb, uuid) TO anon;
GRANT EXECUTE ON FUNCTION public.create_public_order(uuid, text, text, text, text, jsonb, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.create_public_order(uuid, text, text, text, text, jsonb, uuid) TO service_role;
