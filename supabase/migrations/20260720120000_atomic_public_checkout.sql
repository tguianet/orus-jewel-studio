-- =============================================================================
-- Checkout atômico público (sem pagamento)
-- Cria order + order_items em uma única transação via create_public_order.
...
BEGIN
  ...
  -- Rejeitar chaves financeiras/status enviadas pelo cliente (defesa em profundidade)
  IF EXISTS (
    SELECT 1
  ...
  -- Produtos duplicados: consolidar quantidades (temp por sessão/transação)
  ...
  -- Validar produtos + preços do banco e acumular subtotal
  ...
  -- Criar pedido (status sempre new; preços só do servidor)
...
-- Permissões: revogar genérico e conceder só o necessário
