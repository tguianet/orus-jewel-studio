...
BEGIN
  ...
  base_slug := regexp_replace(lower(unaccent(coalesce(display, split_part(NEW.email,'@',1)))), '[^a-z0-9]+', '-', 'g');
...
EXCEPTION WHEN undefined_function THEN
  -- unaccent not installed; fall back without accent stripping
...
CREATE UNIQUE INDEX IF NOT EXISTS seller_stores_slug_unique ON public.seller_stores (store_slug);
