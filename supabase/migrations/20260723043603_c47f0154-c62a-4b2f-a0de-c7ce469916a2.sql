-- Remove a assinatura antiga (6 argumentos)
DROP FUNCTION IF EXISTS public.create_public_order(uuid, text, text, text, text, jsonb);

-- Recria a assinatura de 7 argumentos com proteções defensivas
CREATE OR REPLACE FUNCTION public.create_public_order(
  ...
  p_checkout_token uuid DEFAULT NULL::uuid
)
RETURNS TABLE(order_id uuid, status public.order_status, subtotal numeric, total numeric, created_at timestamp with time zone, items jsonb)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF FOUND THEN
    ...
    RETURN QUERY SELECT v_order_id, v_status, v_subtotal, v_total, v_created_at, v_items;
    RETURN;
  END IF;

  -- Reinicializa após lookup (garante que não fica NULL vindo do SELECT INTO sem match)
  v_subtotal := 0;
  v_total := 0;
  ...
  IF EXISTS (
    ...
    WHERE el ? 'unit_price' OR el ? 'total' OR el ? 'subtotal'
       OR el ? 'status' OR el ? 'cost_price' OR el ? 'wholesale_price' OR el ? 'commission'
  ) THEN
  ...
  LOOP
    ...
    SELECT quantity INTO v_qty FROM tmp_public_order_items WHERE product_id = v_product_id;

    IF v_qty > c_max_qty_per_item THEN
      RAISE EXCEPTION 'Quantidade consolidada excede o máximo de % para o produto %', c_max_qty_per_item, v_product_id;
    END IF;
  END LOOP;
  ...
  -- Cálculo defensivo do subtotal
  FOR v_product_id, v_qty IN
    SELECT t.product_id, t.quantity FROM tmp_public_order_items t
  LOOP
    v_product_name := NULL;
    v_product_status := NULL;
    v_link_active := NULL;
    v_unit_price := NULL;

    SELECT p.name, p.status, sp.active, COALESCE(sp.resale_price, p.suggested_price)
      INTO v_product_name, v_product_status, v_link_active, v_unit_price
    FROM public.products p
    INNER JOIN public.store_products sp
      ON sp.product_id = p.id AND sp.seller_store_id = p_seller_store_id
    ...
    IF v_unit_price IS NULL THEN
      RAISE EXCEPTION 'Preço não definido para o produto "%"', v_product_name;
    END IF;
    ...
    v_line_total := round(COALESCE(v_unit_price, 0) * v_qty, 2);

    IF v_line_total IS NULL THEN
      RAISE EXCEPTION 'Falha ao calcular total do item "%"', v_product_name;
    END IF;

    v_subtotal := COALESCE(v_subtotal, 0) + COALESCE(v_line_total, 0);
  END LOOP;

  -- Proteção crítica antes do INSERT
  ...
  BEGIN
    INSERT INTO public.orders (
      ...
      subtotal, discount, total, status, origin, checkout_token
    ) VALUES (
      p_seller_store_id,
  ...
  LOOP
    ...
    FROM public.products p
    INNER JOIN public.store_products sp
      ON sp.product_id = p.id AND sp.seller_store_id = p_seller_store_id AND sp.active = true
    WHERE p.id = v_product_id AND p.status = 'active'::public.product_status;
