-- =============================================================================
-- Admin: classificação em massa de products.jewelry_material
-- Lovable Cloud — NÃO aplicar via CLI; aplicar manualmente no Cloud.
-- Altera SOMENTE jewelry_material (+ updated_at). Não muda preço/estoque/status.
-- =============================================================================

-- Resumo para o painel admin
...
BEGIN
  ...
  SELECT
    ...
    COUNT(*) FILTER (
      WHERE jewelry_material IS NULL AND status = 'active'::public.product_status
    )::integer,
    COUNT(*) FILTER (
      WHERE jewelry_material IS NULL AND status = 'inactive'::public.product_status
    )::integer
  INTO
...
REVOKE ALL ON FUNCTION public.admin_get_jewelry_material_summary() FROM PUBLIC, anon;
...
-- Classificação em lote (máx. 100 IDs por chamada)
...
BEGIN
  ...
  -- Dedup + limite por lote (evita timeout / payload abusivo)
  ...
  -- Snapshot antes (amostra + contagem) — só catálogo atacado
  ...
  -- Única coluna de negócio alterada: jewelry_material (+ updated_at técnico)
  ...
  -- IDs que já tinham o valor (ou foram atualizados) — amostra para audit
  SELECT ARRAY(
...
REVOKE ALL ON FUNCTION public.admin_bulk_set_jewelry_material(uuid[], text) FROM PUBLIC, anon;
