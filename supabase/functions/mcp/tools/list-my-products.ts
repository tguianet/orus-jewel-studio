...
export default defineTool({
  ...
  title: "Meus produtos",
  description:
    "Lista produtos ativos na loja da sacoleira autenticada. Respeita RLS.",
  inputSchema: {
    limit: z
      .number()
      .int()
      .positive()
      .optional()
      .describe("Máximo de itens (padrão 50)."),
  },
  ...
  handler: async ({ limit }, ctx) => {
    ...
    const { data, error } = await sb
      .from("store_products")
      .select(
        "id, resale_price, active, product:products(id, code, name, category_name, image_url, status)",
      )
