  ...
  return (
    <>
        ...
        return (
          ...
          >
            <div className="container flex flex-wrap items-center justify-center gap-x-10 gap-y-2 py-4 text-[12px] uppercase tracking-wider">
              {benefits.map((b, i) => (
                <span key={i} className={`font-medium ${i >= 2 ? (narrowPreview ? "hidden" : "hidden md:inline") : ""}`}>{b}</span>
              ))}
            </div>
          </section>
        );
      ...
      {(t.showCollections ?? true) && (
        ...
        >
          ...
          {collections.length === 0 ? (
            ...
            <div className={`grid gap-4 sm:gap-6 ${narrowPreview ? "grid-cols-2" : "grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6"}`}>
              {collections.map((c) => {
      ...
      <section id="vitrine" className="container pt-4 pb-14 sm:pb-20 scroll-mt-20">
        ...
        <div className={`${narrowPreview ? "flex" : "sm:hidden"} mb-6 justify-center`}>
          <button
        ...
        <div className={`${narrowPreview ? "hidden" : "hidden sm:flex"} gap-1 overflow-x-auto pb-6 -mx-4 px-4 mb-10 justify-center`}>
          {cats.map((c) => (
            <button
              key={c}
        ...
        {productsLoading ? (
          ...
          <>
          <div className={narrowPreview ? "grid grid-cols-3 gap-2" : "grid grid-cols-3 gap-2 sm:hidden"}>
            {filtered.map((p) => (
              <button
                key={p.id}
                type="button"
          ...
          <div className={narrowPreview ? "hidden" : "hidden sm:grid gap-x-6 gap-y-12 sm:grid-cols-2 lg:grid-cols-4"}>
            {filtered.map((p) => (
      ...
      >
        ...
        <div>
          ...
          <div className={narrowPreview ? "grid grid-cols-1 gap-5" : "grid md:grid-cols-3 gap-5"} data-testid="store-materials-grid">
        ...
        <div className={narrowPreview ? "grid grid-cols-1 gap-5" : "grid md:grid-cols-3 gap-5"}>
