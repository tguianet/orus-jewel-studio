...
export default defineTool({
  ...
  inputSchema: {
    ...
    limit: z.number().int().positive().optional().describe("Máximo de pedidos (padrão 20)."),
  },
  ...
  handler: async ({ status, limit }, ctx) => {
    ...
    let q = sb.from("orders")
      .select("id, created_at, status, customer_name, customer_phone, subtotal, discount, total:subtotal, notes")
