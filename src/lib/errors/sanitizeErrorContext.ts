...
const ALLOWED_KEY =
  /^(order_id|withdrawal_id|return_id|reseller_id|store_id|rpc_name|route|operation|status|http_status|error_code|correlation_id|entity_type|entity_id|severity|category|code|retryable|page|page_size|signup_stage|auth_error_code|postgres_code|constraint|referral_reason|template_key|pg_code|pg_details|pg_hint)$/i;
